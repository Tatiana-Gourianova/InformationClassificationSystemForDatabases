...
-- -----------------------------------------------------
-- -----------------------------------------------------
-- Schema transacoustic
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema transacoustic
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `transacoustic` ;
USE `transacoustic` ;
...
CREATE TABLE IF NOT EXISTS `transacoustic`.`constructs` (
  `Id` INT NULL DEFAULT NULL AUTO_INCREMENT COMMENT ' Id типа.Первичный синтетический ключ таблицы.',
...
CREATE TABLE IF NOT EXISTS `transacoustic`.`materials` (
  `Id` INT NULL DEFAULT NULL AUTO_INCREMENT COMMENT ' Id юнита.Первичный синтетический ключ таблицы.',
...
CREATE TABLE IF NOT EXISTS `transacoustic`.`items` (
  `Id` INT NULL DEFAULT NULL AUTO_INCREMENT COMMENT ' Id cлоя-\"сенсора\".Первичный синтетический ключ таблицы.',
  `Name` VARCHAR(250) NOT NULL,
  `Thickness` DECIMAL(6,2) NULL DEFAULT NULL,
  `Density` DECIMAL(10,3) NULL DEFAULT NULL,
  `Si63` DECIMAL NULL DEFAULT NULL COMMENT 'одно поле для одного значения звукоизоляции отсортированы в настоящее время, чтобы удобно было вводить прямо в базу',
  ...
  `Sound_Insulations` VARCHAR(60) NULL DEFAULT NULL COMMENT 'дублирование значений Sound_Insulations в виде строки ',
  `Constructs_Id` INT NULL DEFAULT NULL COMMENT ' Внешний ключ таблицы для связи с таблицей  Constructions',
  `AveragedSoundInsulation` DECIMAL NULL DEFAULT NULL,
  `Constructions_Id` INT NULL DEFAULT NULL,
  `Characteristics_Id` INT NULL DEFAULT NULL COMMENT 'наименования колонок Sound_Insulations нужно, если воодить сразу строку',
  ...
  `Units_Id` VARCHAR(200) NULL DEFAULT NULL COMMENT 'комментарий для информации, которую не понятно, куда добавлять',
  ...
  CONSTRAINT `fk_items_materials1`
    ...
    ON UPDATE NO ACTION)
DEFAULT CHARACTER SET = utf8mb3
COMMENT = 'Таблица пополнения из базы данных. На данном этапе это материалы из INSUL 3333333333333_1710413376.pdf Основной элемент системы заполнения. ';
...
CREATE TABLE IF NOT EXISTS `transacoustic`.`roles` (
  `Id` INT NULL DEFAULT NULL AUTO_INCREMENT COMMENT ' Id роли.Первичный синтетический ключ таблицы.',
...
CREATE TABLE IF NOT EXISTS `transacoustic`.`users` (
  `Id` INT NULL DEFAULT NULL AUTO_INCREMENT COMMENT ' Поле id пользователя.Первичный синтетический ключ таблицы.',
  `Login` VARCHAR(41) NOT NULL COMMENT ' Логин пользователя для входа в систему.Также является альтернативным ключом или уникальным индексом.',
